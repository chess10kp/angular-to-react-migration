...
| C-L1 | C.2 · **build sheet C.8** | C-L0 | TODO | Automate the **type-oracle gate** (impl `Oracle` kind:type): `tsc` reads **stdout**, `-p tsconfig`, normalize `.component.ts↔.tsx` paths, map by `span` line-range, **pass = baseline-diff** (no new errors). Transcribe C.8/C-L1. |
| C-L2 | C.1 · **build sheet C.8** | C-L1 | TODO | Build the **loop spine + contracts.ts + Picker + Committer + RetryPolicy** (§C.1.5, `tsx`/`.mts`): pick → retrieve → apply → **git-status allowlist firewall** → verify → **append-lesson-then-commit** (one tree). Status in `status.jsonl` sidecar. Transcribe C.8/C-L2. |
